...
function createRoom() {
  ...
  rooms[code] = {
    ...
    teams: Array.from({ length: MAX_TEAMS }, (_, i) => ({
      ...
      color: TEAM_COLORS[i],
      connected: false,
      everJoined: false, // pernah join setidaknya sekali -> slot "milik" peserta ini
      socketId: null,
    ...
    joinRequests: [], // [{ id, teamId, name, socketId }] permintaan masuk kembali yang menunggu approval admin
    spectator: { connected: false, socketId: null }, // hanya 1 device spectator per room
  };
  return rooms[code];
}
...
function roomStateForAdmin(room) {
  return {
    ...
    joinRequests: room.joinRequests.map(jr => ({ id: jr.id, teamId: jr.teamId, name: jr.name })),
  };
}
...
function broadcastRoom(room) {
  ...
  if (room.spectator.connected && room.spectator.socketId) {
    io.to(room.spectator.socketId).emit('state', roomStateForSpectator(room));
  }
}

// State yang dikirim ke SPECTATOR (proyektor): 6 papan kecil per tim + leaderboard,
// sama seperti view peserta (hit/miss saja, tidak ada posisi kapal yang belum kena)
function roomStateForSpectator(room) {
  return {
    code: room.code,
    phase: room.phase,
    teams: room.teams.map(t => ({
      ...publicTeam(t),
      cellsRevealed: revealedCellsForTeam(room, t.id),
    })),
    activeTeamId: room.activeTeamId,
    pendingFire: room.pendingFire,
    log: room.log.slice(-10),
...
// Papan kecil per-tim untuk spectator: grid SIZE x SIZE berisi 'hit' | 'miss' | null
// 'hit' hanya ditandai pada sel milik tim ini; 'miss' ditandai jika koordinat itu pernah
// ditembak dan ternyata tidak mengenai kapal manapun (bersifat sama di semua papan kecil,
// karena papan tiap tim memakai sistem koordinat global yang sama).
function revealedCellsForTeam(room, teamId) {
  const grid = Array.from({ length: SIZE }, () => Array(SIZE).fill(null));
  for (let r = 0; r < SIZE; r++) {
    for (let c = 0; c < SIZE; c++) {
      const state = room.revealed[r][c];
      if (!state) continue;
      if (state === 'miss') {
        grid[r][c] = 'miss';
      } else if (state === 'hit' && room.board[r][c].includes(teamId)) {
        grid[r][c] = 'hit';
      }
    }
  }
...
io.on('connection', (socket) => {
  ...
  // ── SPECTATOR: gabung sebagai layar proyektor (hanya 1 device per room) ──
  socket.on('spectator:join', ({ code }) => {
    ...
    if (room.spectator.connected) {
      socket.emit('spectator:full', 'Mode spectator sudah digunakan oleh device lain di room ini. Hanya 1 layar spectator yang diizinkan per game.');
      return;
    }
    ...
    socket.join(`${room.code}:spectator`);
    socket.data.roomCode = room.code;
    socket.data.role = 'spectator';

    addLog(room, `📺 Layar spectator terhubung.`);
    socket.emit('spectator:joined', { code: room.code });
    socket.emit('state', roomStateForSpectator(room));
    broadcastRoom(room);
  });
  ...
  socket.on('player:join', ({ code, name }) => {
    ...
    if (!cleanName) {
      socket.emit('errorMsg', 'Masukkan nama tim.');
      return;
    }

    // Cek apakah sudah ada permintaan join yang sama persis tertunda
    const alreadyRequested = room.joinRequests.find(jr => jr.name.toLowerCase() === cleanName.toLowerCase());
    if (alreadyRequested) {
      socket.data.roomCode = room.code;
      socket.data.role = 'player';
      socket.data.pendingJoinId = alreadyRequested.id;
      alreadyRequested.socketId = socket.id; // update socket terbaru jika user reload saat menunggu
      socket.emit('player:waitingApproval', { name: cleanName });
      return;
    }

    // 1) Cocokkan ke slot yang PERNAH dipakai dengan nama yang sama (peserta lama yang keluar/disconnect)
    let matchedSlot = room.teams.find(t =>
      t.everJoined && t.name.toLowerCase() === cleanName.toLowerCase()
    );

    if (matchedSlot) {
      if (matchedSlot.connected) {
        socket.emit('errorMsg', 'Nama ini sedang aktif terhubung dari device lain. Jika itu kamu, tutup tab lama terlebih dahulu.');
        return;
      }
      // Buat permintaan masuk kembali -> menunggu approval admin
      const reqId = crypto.randomUUID();
      room.joinRequests.push({ id: reqId, teamId: matchedSlot.id, name: cleanName, socketId: socket.id });
      socket.data.roomCode = room.code;
      socket.data.role = 'player';
      socket.data.pendingJoinId = reqId;

      addLog(room, `🔔 ${cleanName} meminta untuk masuk kembali ke room (sebagai ${matchedSlot.name}).`);
      socket.emit('player:waitingApproval', { name: cleanName });
      broadcastRoom(room);
      return;
    }

    // 2) Tidak ada slot lama dengan nama ini -> coba ambil slot kosong (belum pernah dipakai sama sekali)
    let team = room.teams.find(t => !t.everJoined);
    if (!team) {
      socket.emit('errorMsg', 'Room penuh (maksimal 6 peserta), dan tidak ditemukan slot dengan nama tersebut.');
      return;
    }

    team.connected = true;
    team.everJoined = true;
  ...
  // ── PESERTA: batalkan permintaan masuk kembali yang masih menunggu approval ──
  socket.on('player:cancelJoinRequest', () => {
    const room = getMyRoom(socket);
    if (!room) return;
    if (!socket.data.pendingJoinId) return;

    const idx = room.joinRequests.findIndex(jr => jr.id === socket.data.pendingJoinId);
    if (idx !== -1) {
      const req = room.joinRequests[idx];
      room.joinRequests.splice(idx, 1);
      addLog(room, `↩️ ${req.name} membatalkan permintaan masuk kembali.`);
      broadcastRoom(room);
    }
    socket.data.pendingJoinId = null;
  });

  // ── ADMIN: setujui permintaan masuk kembali ──
  socket.on('admin:approveJoin', ({ requestId }) => {
    const room = getMyRoom(socket);
    if (!room || socket.data.role !== 'admin') return;

    const idx = room.joinRequests.findIndex(jr => jr.id === requestId);
    if (idx === -1) return;
    const req = room.joinRequests[idx];
    room.joinRequests.splice(idx, 1);

    const team = room.teams[req.teamId];
    const targetSocket = io.sockets.sockets.get(req.socketId);
    if (!targetSocket) {
      addLog(room, `⚠️ ${req.name} sudah tidak terhubung saat disetujui.`);
      broadcastRoom(room);
      return;
    }

    team.connected = true;
    team.socketId = targetSocket.id;
    team.name = req.name;

    targetSocket.join(`${room.code}:player:${team.id}`);
    targetSocket.data.roomCode = room.code;
    targetSocket.data.role = 'player';
    targetSocket.data.teamId = team.id;
    targetSocket.data.pendingJoinId = null;

    addLog(room, `✅ ${team.name} disetujui masuk kembali ke room.`);
    targetSocket.emit('player:joined', { teamId: team.id, name: team.name, color: team.color });
    broadcastRoom(room);
  });

  // ── ADMIN: tolak permintaan masuk kembali ──
  socket.on('admin:rejectJoin', ({ requestId }) => {
    const room = getMyRoom(socket);
    if (!room || socket.data.role !== 'admin') return;
    ...
    if (targetSocket) {
      targetSocket.data.pendingJoinId = null;
      targetSocket.emit('errorMsg', 'Permintaan untuk masuk kembali ditolak oleh juri.');
      targetSocket.emit('player:rejected');
    }

    addLog(room, `🚫 Permintaan ${req.name} untuk masuk kembali ditolak.`);
    broadcastRoom(room);
  });

  // ── ADMIN: kick peserta (kosongkan slot agar bisa dipakai peserta baru) ──
  socket.on('admin:kickPlayer', ({ teamId }) => {
    const room = getMyRoom(socket);
    if (!room || socket.data.role !== 'admin') return;
    const team = room.teams[teamId];
    if (!team) return;

    const kickedName = team.name;

    // Putuskan koneksi socket peserta jika sedang terhubung
    if (team.socketId) {
      const targetSocket = io.sockets.sockets.get(team.socketId);
      if (targetSocket) {
        targetSocket.emit('player:kicked');
        targetSocket.data.roomCode = null;
        targetSocket.data.teamId = undefined;
        targetSocket.leave(`${room.code}:player:${team.id}`);
      }
    }

    // Buang sisa permintaan join dari nama ini juga
    room.joinRequests = room.joinRequests.filter(jr => jr.teamId !== team.id);

    // Hapus posisi kapal dari board jika sudah ditempatkan
    if (team.placed && team.cells) {
      team.cells.forEach(([rr, cc]) => {
        room.board[rr][cc] = room.board[rr][cc].filter(id => id !== team.id);
      });
    }

    // Reset slot ke kondisi kosong total (siap dipakai peserta baru)
    room.teams[teamId] = {
      id: teamId,
      slot: teamId,
      name: `Tim ${teamId + 1}`,
    ...
    addLog(room, `👋 ${kickedName} dikeluarkan dari room oleh juri. Slot dikosongkan.`);
    broadcastRoom(room);
  });
  ...
  socket.on('admin:resetRoom', () => {
    ...
    oldTeams.forEach((t, i) => {
      if (t.connected && t.socketId) {
        ...
        rooms[code].teams[i].everJoined = true;
        rooms[code].teams[i].socketId = t.socketId;
        rooms[code].teams[i].name = t.name;
      }
    });

    // pertahankan koneksi spectator yang sedang online
    if (room.spectator.connected && room.spectator.socketId) {
      rooms[code].spectator.connected = true;
      rooms[code].spectator.socketId = room.spectator.socketId;
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (socket.data.role === 'spectator' && room.spectator.socketId === socket.id) {
      room.spectator.connected = false;
      room.spectator.socketId = null;
      addLog(room, `📺 Layar spectator terputus.`);
      broadcastRoom(room);
    }
    if (socket.data.pendingJoinId) {
      const idx = room.joinRequests.findIndex(jr => jr.id === socket.data.pendingJoinId);
      if (idx !== -1) {
        const req = room.joinRequests[idx];
        room.joinRequests.splice(idx, 1);
        addLog(room, `↩️ Permintaan masuk dari ${req.name} dibatalkan (terputus saat menunggu).`);
        broadcastRoom(room);
      }
    }
  });
