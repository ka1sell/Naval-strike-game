...
- **Spectator (opsional)**: buka `/spectator.html` di 1 device (untuk proyektor), masukkan **kode room**.
...
## Mode Spectator (Layar Proyektor)

- Buka `/spectator.html`, masukkan kode room — cocok untuk ditampilkan di proyektor/layar besar.
- **Hanya 1 device spectator** yang diizinkan per room. Jika device lain mencoba membuka mode spectator di room yang sama saat slot masih terpakai, akan muncul pesan error dan ditolak.
- Tampilan: 6 papan kecil milik tiap tim (hanya menampilkan hasil hit/miss, **tidak** menampilkan posisi kapal yang belum tertembak — sama seperti yang dilihat peserta lain), leaderboard real-time, dan riwayat pertempuran. Read-only, tanpa kontrol apapun.
- Spectator otomatis mendapat update setiap kali ada perubahan status (tembakan, skor, peserta gugur, dll).
- Jika spectator disconnect (tab ditutup/refresh), slot otomatis terbuka kembali dan bisa diisi device lain.
...
5. Setelah deploy, akses domain Railway, lalu:
   ...
   - 1 layar proyektor buka `https://<domain>/spectator.html`.
...
└── public/
    ...
    └── spectator.html  # layar proyektor (read-only)
```
